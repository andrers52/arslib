...
export class EFunction {
  ...
  //       be used.
  static sequence = (f: any, g: any, context: any) => {
  ...
  static compose = (f: any, g: any, context: any) => {
    Assert.assertIsFunction(f);
    Assert.assertIsFunction(g);
    Assert.assert(context, "No function running context found.");
    let self = context || this;
  ...
  */
   ...
   */
  static memoize = (f: any, maxEntries: any = 1000000) => {
    let cache = {};
    let count = 0;
    return (arg) => {
   ...
   */
  static limitCallingRateWithDiscard = (f: any, delay: any) => {
    let canCall = true;
    return (arg) => {
      if (!canCall) return;
      canCall = false;
   ...
   */
  static addRuntimeTypeTest = (fn: any, argsTypesArray: any, resultType: any, context: any) => {
    return (...args) => {
  ...
  static registerObserver = (observedFnc: any, observerFnc: any) => {
    let result;
    if (observedToObservers[observedFnc.id]) {
      ...
      result = (arg) => {
        observedFnc(arg);
        EFunction.runObservers(observedFnc.id);
      };
    }
    return result;
  };
  ...
  static unregisterObserver = (observedFnc: any, observerFnc: any) => {
