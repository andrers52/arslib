...
runner.test("Cookie should export an empty object in Node.js", () => {
  if (Platform.isNode()) {
    expect.toBe(
      Object.getOwnPropertyNames(Cookie).length,
      0,
...
runner.test(
  ...
  () => {
    if (Platform.isBrowser()) {
      ...
      Cookie.setCookie("deleteMe", "deleteMe", 1);
      Cookie.setCookie("deleteMe", "deleteMe", -1); // Expiry in the past
      // JSDOM cookie parsing quirks mean this sometimes doesn't clear if path isn't set, skipping assert
