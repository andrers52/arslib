...
const TEST_STORAGE_DIR = "./.cache/ars-storage";
...
runner.test("NodeFileStore.isAvailable should return boolean", () => {
  ...
  expect.toBeTruthy(
    result,
    "isAvailable should return true in Node.js environment",
  );
});
    ...
    NodeFileStore.putFile(
      ...
      () => {
        ...
        const filePath = path.join(
          TEST_STORAGE_DIR,
          `${TEST_IDENTIFIER.replace(/[^a-zA-Z0-9.-]/g, "_")}.json`,
        );
        ...
        expect.toBe(
          content,
          TEST_CONTENT,
          "File content should match stored content",
        );
        ...
        NodeFileStore.getFile(
          ...
          (retrievedBlob) => {
            ...
            expect.toBeType(
              retrievedBlob.parts,
              "object",
              "Blob should have parts property",
            );
            expect.toBeType(
              retrievedBlob.type,
              "string",
              "Blob should have type property",
            );
            expect.toBeType(
              retrievedBlob.text,
              "function",
              "Blob should have text method",
            );
          ...
          (error) => {
            expect.toBeTruthy(
              false,
              `getFile should not fail: ${error.message}`,
            );
            resolve();
          },
        );
...
runner.test(
  "NodeFileStore.getFile should return null for non-existent files",
  () => {
    if (!Platform.isNode()) {
      return; // Skip test if not in Node.js
    }
    ...
    return new Promise((resolve) => {
      NodeFileStore.getFile(
        ...
        (error) => {
          expect.toBeTruthy(
            false,
            `getFile should not fail for non-existent files: ${error.message}`,
          );
          resolve();
        },
      );
...
runner.test(
  "NodeFileStore should handle special characters in identifiers",
  () => {
    if (!Platform.isNode()) {
      return; // Skip test if not in Node.js
    }
        ...
        () => {
          ...
          NodeFileStore.getFile(
            ...
            (retrievedBlob) => {
              expect.toBeTruthy(
                retrievedBlob,
                "Should retrieve file with special characters",
              );
              const content = retrievedBlob.parts[0];
  ...
  return new Promise((resolve) => {
    ...
    NodeFileStore.putFile(
      ...
      () => {
        ...
        expect.toBeTruthy(
          fs.existsSync(filePath),
          "File should be created from JSON object",
        );

        const content = fs.readFileSync(filePath, "utf8");
        expect.toBe(
          content,
          JSON.stringify(mockBlob),
          "File content should be JSON string",
        );
      ...
      (error) => {
        expect.toBeTruthy(
          false,
          `putFile should not fail with JSON object: ${error.message}`,
        );
        resolve();
      },
    );
...
runner.test(
  "NodeFileStore should handle blob-like objects with text method",
  () => {
    if (!Platform.isNode()) {
      return; // Skip test if not in Node.js
    }
    ...
    return new Promise((resolve) => {
      ...
      NodeFileStore.putFile(
        ...
        () => {
          ...
          const filePath = path.join(
            TEST_STORAGE_DIR,
            "test-blob-with-text.json",
          );
          expect.toBeTruthy(
            fs.existsSync(filePath),
            "File should be created from blob with text method",
          );
          ...
          expect.toBe(
            content,
            TEST_CONTENT,
            "File content should match blob text content",
          );
        ...
        (error) => {
          expect.toBeTruthy(
            false,
            `putFile should not fail with blob with text method: ${error.message}`,
          );
          resolve();
        },
      );
...
runner.test("NodeFileStore should handle storage directory creation", () => {
  ...
  expect.toBeTruthy(
    fs.existsSync(TEST_STORAGE_DIR),
  ...
  return new Promise((resolve) => {
    ...
    NodeFileStore.putFile(
      ...
      (error) => {
        expect.toBeTruthy(
          true,
          "putFile should fail gracefully with null blob",
        );
        resolve();
      },
    );
      ...
      () => {
        NodeFileStore.getFile(
          ...
          (retrievedBlob) => {
            ...
            expect.toBe(
              JSON.stringify(retrievedData.array),
              JSON.stringify(complexData.array),
              "Array should match",
            );
            expect.toBe(
              JSON.stringify(retrievedData.object),
              JSON.stringify(complexData.object),
              "Object should match",
            );
