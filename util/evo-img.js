...
//index is code
export let drawingCodeToType = ['rectangle', 'circle', 'triangle']
export let globalCompositeOperationCodeToType = [
  'source-over', 'lighter', 'copy', 'xor',
...
function execInstruction (instructionParameters, context) {
  ...
  context.fillStyle = context.strokeStyle = `rgba(${colorR}, ${colorG}, ${colorB}, 1)`
  context.globalAlpha = alpha
  context.globalCompositeOperation = globalCompositeOperationCodeToType[instructionParameters[1]]
  context.lineWidth = lineWidth
  let operation = drawingCodeToType[instructionParameters[0]]
  switch (operation) {
  case 'rectangle':
    if (fill) {
      context.fillRect(positionX - (sizeX / 2), positionY - (sizeY / 2), sizeX, sizeY)
    } else {
      context.rect(positionX - (sizeX / 2), positionY - (sizeY / 2), sizeX, sizeY)
      context.stroke()
    }
    break
