...
class Persistence {
  static configure(namespace, ...entities) {
    ...
    entities.forEach((entity) => {
      // initialize mapping
      Persistence.entitiesToValues[entity] = Persistence.retrieve(entity);
      // create getter and setter
      Object.defineProperty(Persistence, entity, {
        get: () => Persistence.entitiesToValues[entity],
        set: (value) => {
          Persistence.entitiesToValues[entity] = Object.assign({}, value);
          setTimeout(() => {
  ...
  static retrieve(entity) {
    return JSON.parse(
      window.localStorage.getItem(`${Persistence.namespace}:${entity}`),
    );
  }
