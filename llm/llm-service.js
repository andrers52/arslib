...
import { BrowserFileStore } from "../browser/browser-file-store.js";
import { NodeFileStore } from "../node/node-file-store.js";
...
function LLMService() {
  ...
  this.initialize = async function(config) {
    if (!config.llmFunction && !config.modelPath) {
      throw new Error("LLMService requires either llmFunction or modelPath");
    }
    this.config = {
      ...this.config,
      ...config
...
function TransformersLLMService() {
  ...
  this.loadModelFromPath = async function(modelPath) {
    try {
      // Suppress ONNX warnings by setting environment variables before import
      const originalEnv = process.env.ORT_LOGGING_LEVEL;
      const originalEnv2 = process.env.ORT_LOGGING_LEVEL_2;
      process.env.ORT_LOGGING_LEVEL = 'error';
      process.env.ORT_LOGGING_LEVEL_2 = 'error';
      
      // Also suppress console warnings temporarily
      ...
      if (originalEnv !== undefined) {
        process.env.ORT_LOGGING_LEVEL = originalEnv;
      } else {
        delete process.env.ORT_LOGGING_LEVEL;
      }
      if (originalEnv2 !== undefined) {
        process.env.ORT_LOGGING_LEVEL_2 = originalEnv2;
      } else {
        delete process.env.ORT_LOGGING_LEVEL_2;
      }
