...
describe("LLMService", function() {
  ...
  describe("Persistent Storage", function() {
    ...
    it("should handle persistent storage unavailability", async function() {
      ...
      const { NodeFileStore } = await import("../node/node-file-store.js");
  ...
  describe("Generation", function() {
    ...
    it("should handle generation errors gracefully", async function() {
      ...
      assert.strictEqual(parsedResponse.action, "moveUp");
      assert.strictEqual(parsedResponse.parameters[0], 1);
      assert(parsedResponse.reasoning.includes("Default action due to LLM error"));
    });
